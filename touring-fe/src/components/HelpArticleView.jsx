...
import { helpArticles as mockArticles } from '../mockdata/helpData';
...
export default function HelpArticleView() {
  ...
  const handleFeedback = async (helpful) => {
    try {
      ...
      setArticle(prev => {
        const helpfulCount = (prev.helpfulCount || 0) + (helpful ? 1 : 0);
        const notHelpfulCount = (prev.notHelpfulCount || 0) + (!helpful ? 1 : 0);
        const total = Math.max(1, helpfulCount + notHelpfulCount);
        const helpfulnessRate = Math.round((helpfulCount / total) * 100);
        return { ...prev, helpfulCount, notHelpfulCount, helpfulnessRate };
      });
      toast.success('Cảm ơn phản hồi của bạn!', { icon: helpful ? '👍' : '👎' });
    } catch (error) {
