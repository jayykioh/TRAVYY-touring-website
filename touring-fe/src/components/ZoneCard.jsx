...
export default function ZoneCard({ zone, onExplore, index = 0, size = "md" }) {
  ...
  const btnPad = isSm ? "px-2.5 py-1.5 text-[11px]" : "px-3 py-2 text-xs";

  // ✅ Calculate match % from finalScore
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-2xl border border-slate-200/70 bg-white/80 backdrop-blur-md shadow-sm hover:shadow-lg transition-shadow duration-300"
    >
      {/* Hero Image */}
      <div className={`relative ${heroH} rounded-t-2xl overflow-hidden`}>
        <div
          className="absolute inset-0 bg-center bg-cover"
          style={{ backgroundImage: heroSrc ? `url(${encodeURI(heroSrc)})` : undefined }}
        />
        {!heroSrc && (
          <div className="absolute inset-0 bg-[linear-gradient(135deg,#0f172acc,#334155cc)]" />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent" />

        {/* ✅ Match Badge - FIXED: check finalScore instead of matchScore */}
        {matchPercent !== null && (
         <motion.span
      ...
      <div className={`flex items-center justify-between ${pad}`}>
        ...
        <motion.button
          ...
          className={`inline-flex items-center gap-1 rounded-xl bg-slate-900 text-white ${btnPad} font-semibold hover:bg-slate-700 transition-all shadow-sm`}
        >
