// heroData.js - Tour travel data for Vietnam destinations

export const heroSlides = [
  {
    id: 1,
    location: "Hội An, Quảng Nam",
    title: "Khám Phá Phố Cổ Hội An",
    subtitle: "Trải nghiệm vẻ đẹp cổ kính và văn hóa độc đáo của phố cổ Hội An với những con phố rực rỡ đèn lồng vàng.",
    rating: 4.9,
    reviews: 12500,
    duration: "2-3 giờ",
    category: "Văn hóa lịch sử",
    price: "từ 650.000₫",
    images: {
      desktop: "https://res.cloudinary.com/dzjm0cviz/image/upload/v1759336726/1_thanh_pho_hoi_an_duoc_unesco_cong_nhan_la_di_san_van_hoa_the_gioi_noi_tieng_voi_nhung_ngoi_nha_co_kinh_62c7183de4_ujxgbq.jpg",
      webp: "https://res.cloudinary.com/dzjm0cviz/image/upload/v1759336726/1_thanh_pho_hoi_an_duoc_unesco_cong_nhan_la_di_san_van_hoa_the_gioi_noi_tieng_voi_nhung_ngoi_nha_co_kinh_62c7183de4_ujxgbq.jpg",
      tablet: "https://res.cloudinary.com/dzjm0cviz/image/upload/v1759336726/1_thanh_pho_hoi_an_duoc_unesco_cong_nhan_la_di_san_van_hoa_the_gioi_noi_tieng_voi_nhung_ngoi_nha_co_kinh_62c7183de4_ujxgbq.jpg",
      mobile: "https://res.cloudinary.com/dzjm0cviz/image/upload/v1759336726/1_thanh_pho_hoi_an_duoc_unesco_cong_nhan_la_di_san_van_hoa_the_gioi_noi_tieng_voi_nhung_ngoi_nha_co_kinh_62c7183de4_ujxgbq.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/9nYDcx31DlU?si=S1xHbe8MBE_9tYhB"
  },
  {
    id: 2,
    location: "Đà Nẵng, Việt Nam",
    title: "Bãi Biển Mỹ Khê Tuyệt Đẹp",
    subtitle: "Thư giãn trên một trong những bãi biển đẹp nhất thế giới với làn nước trong xanh và cát trắng mịn màng.",
    rating: 4.8,
    reviews: 8900,
    duration: "Cả ngày",
    category: "Biển đảo",
    price: "từ 850.000₫",
    images: {
      desktop: "https://i.pinimg.com/736x/79/8f/c4/798fc4327101187f2f647949f3ef5284.jpg",
      webp: "https://i.pinimg.com/736x/79/8f/c4/798fc4327101187f2f647949f3ef5284.jpg",
      tablet: "https://i.pinimg.com/736x/79/8f/c4/798fc4327101187f2f647949f3ef5284.jpg",
      mobile: "https://i.pinimg.com/736x/79/8f/c4/798fc4327101187f2f647949f3ef5284.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/8BfymkVZfww?si=eN_eFY7n1bRLcolG"
  },
  {
    id: 3,
    location: "Sa Pa, Lào Cai",
    title: "Ruộng Bậc Thang Sa Pa",
    subtitle: "Chiêm ngưỡng vẻ đẹp hùng vĩ của ruộng bậc thang và khám phá văn hóa đa dạng các dân tộc thiểu số.",
    rating: 4.9,
    reviews: 15200,
    duration: "2-3 ngày",
    category: "Thiên nhiên",
    price: "từ 2.990.000₫",
    images: {
      desktop: "https://i.pinimg.com/736x/39/f2/08/39f208ed0cdb75defa9ca2b60c0527e8.jpg",
      webp: "https://i.pinimg.com/736x/39/f2/08/39f208ed0cdb75defa9ca2b60c0527e8.jpg",
      tablet: "https://i.pinimg.com/736x/39/f2/08/39f208ed0cdb75defa9ca2b60c0527e8.jpg",
      mobile: "https://i.pinimg.com/736x/39/f2/08/39f208ed0cdb75defa9ca2b60c0527e8.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/dHf8-s-gsEA?si=EuU7eh1K7ALXQzrK"
  },
  {
    id: 4,
    location: "Vịnh Hạ Long, Quảng Ninh",
    title: "Du Ngoạn Vịnh Hạ Long",
    subtitle: "Khám phá kỳ quan thiên nhiên thế giới với hàng nghìn hòn đảo đá vôi kỳ thú và những hang động bí ẩn.",
    rating: 4.8,
    reviews: 18750,
    duration: "1-2 ngày",
    category: "Kỳ quan thiên nhiên",
    price: "từ 1.590.000₫",
    images: {
      desktop: "https://images.unsplash.com/photo-1528127269322-539801943592?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
      webp: "https://images.unsplash.com/photo-1528127269322-539801943592?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
      tablet: "https://images.unsplash.com/photo-1528127269322-539801943592?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
      mobile: "https://images.unsplash.com/photo-1528127269322-539801943592?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80"
    },
    videoUrl: "https://www.youtube.com/embed/vt9OL_sJ5gA?si=p8_48wFeWndsh-8G"

  },
  {
    id: 5,
    location: "Phú Quốc, Kiên Giang",
    title: "Đảo Ngọc Phú Quốc",
    subtitle: "Tận hưởng thiên đường nghỉ dưỡng với những bãi biển cát trắng, nước biển trong như pha lê và hải sản tươi ngon.",
    rating: 4.7,
    reviews: 11400,
    duration: "3-5 ngày",
    category: "Nghỉ dưỡng",
    price: "từ 3.450.000₫",
    images: {
      desktop: "https://i.pinimg.com/1200x/e4/83/68/e4836889b026dade1726f7101b6c4f09.jpg",
      webp: "https://i.pinimg.com/1200x/e4/83/68/e4836889b026dade1726f7101b6c4f09.jpg",
      tablet: "https://i.pinimg.com/1200x/e4/83/68/e4836889b026dade1726f7101b6c4f09.jpg",
      mobile: "https://i.pinimg.com/1200x/e4/83/68/e4836889b026dade1726f7101b6c4f09.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/EIMRO50r09I?si=a8iqPHdMsOFGZ5s0"

  },
  {
    id: 6,
    location: "Ninh Bình, Việt Nam",
    title: "Tràng An Hoa Lư",
    subtitle: "Khám phá 'Vịnh Hạ Long trên cạn' với những dòng sông êm đềm uốn lượn qua hang động và núi đá hùng vĩ.",
    rating: 4.8,
    reviews: 9650,
    duration: "1 ngày",
    category: "Sinh thái",
    price: "từ 790.000₫",
    images: {
      desktop: "https://images.unsplash.com/photo-1545389336-cf090694435e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2128&q=80",
      webp: "https://images.unsplash.com/photo-1545389336-cf090694435e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2128&q=80&fm=webp",
      tablet: "https://images.unsplash.com/photo-1545389336-cf090694435e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1024&q=80",
      mobile: "https://images.unsplash.com/photo-1545389336-cf090694435e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=768&q=80"
    },
    videoUrl: "https://www.youtube.com/embed/KMJXtZb6IUk?si=ZTgXI203C3FzpOAU"

  },
  {
    id: 7,
    location: "Hà Nội, Việt Nam",
    title: "Phố Cổ Hà Nội",
    subtitle: "Dạo bước khám phá 36 phố phường với lịch sử hàng nghìn năm, thưởng thức ẩm thực đường phố đặc sắc.",
    rating: 4.6,
    reviews: 13200,
    duration: "4-6 giờ",
    category: "Văn hóa đô thị",
    price: "từ 450.000₫",
    images: {
      desktop: "https://i.pinimg.com/1200x/db/71/4e/db714eb743f61d88965cddd7b07cfa28.jpg",
      webp: "https://i.pinimg.com/1200x/db/71/4e/db714eb743f61d88965cddd7b07cfa28.jpg",
      tablet: "https://i.pinimg.com/1200x/db/71/4e/db714eb743f61d88965cddd7b07cfa28.jpg",
      mobile: "https://i.pinimg.com/1200x/db/71/4e/db714eb743f61d88965cddd7b07cfa28.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/MT8vPndyDa8?si=e_AmyTjC-bXAAl2G"

  },
  {
    id: 8,
    location: "Đà Lạt, Lâm Đồng",
    title: "Thành Phố Ngàn Hoa",
    subtitle: "Tận hưởng khí hậu mát mẻ quanh năm, ngắm hoa và thưởng thức cà phê đặc sản tại thành phố sương mù.",
    rating: 4.7,
    reviews: 14800,
    duration: "2-3 ngày",
    category: "Nghỉ dưỡng",
    price: "từ 1.850.000₫",
    images: {
      desktop: "https://i.pinimg.com/736x/76/0b/61/760b6133d946214cba05b61f5f5f3817.jpg",
      webp: "https://i.pinimg.com/736x/76/0b/61/760b6133d946214cba05b61f5f5f3817.jpg",
      tablet: "https://i.pinimg.com/736x/76/0b/61/760b6133d946214cba05b61f5f5f3817.jpg",
      mobile: "https://i.pinimg.com/736x/76/0b/61/760b6133d946214cba05b61f5f5f3817.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/MDJrgQHaNG4?si=9U9_WjZ4UlTR2sG9"

  },
  {
    id: 9,
    location: "TP. Hồ Chí Minh",
    title: "Sài Gòn Năng Động",
    subtitle: "Khám phá nhịp sống sôi động của thành phố không ngủ với ẩm thực phong phú và văn hóa đa dạng.",
    rating: 4.5,
    reviews: 16900,
    duration: "2-4 ngày",
    category: "Đô thị hiện đại",
    price: "từ 1.290.000₫",
    images: {
      desktop: "https://images.unsplash.com/photo-1583417319070-4a69db38a482?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80",
      webp: "https://images.unsplash.com/photo-1583417319070-4a69db38a482?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2070&q=80&fm=webp",
      tablet: "https://images.unsplash.com/photo-1583417319070-4a69db38a482?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1024&q=80",
      mobile: "https://images.unsplash.com/photo-1583417319070-4a69db38a482?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=768&q=80"
    },
    videoUrl: "https://www.youtube.com/embed/uUL6pCWAX78?si=BAavz38QPVdwB84j"

  },
  {
    id: 10,
    location: "Mũi Né, Bình Thuận",
    title: "Đồi Cát Bay Mũi Né",
    subtitle: "Trải nghiệm cảm giác trượt cát trên những đồi cát đỏ và vàng tuyệt đẹp như sa mạc thu nhỏ.",
    rating: 4.6,
    reviews: 7850,
    duration: "1-2 ngày",
    category: "Phiêu lưu",
    price: "từ 990.000₫",
    images: {
      desktop: "https://i.pinimg.com/1200x/e0/05/53/e005534fae6501adb82e077161b0a67c.jpg",
      webp: "https://i.pinimg.com/1200x/e0/05/53/e005534fae6501adb82e077161b0a67c.jpg",
      tablet: "https://i.pinimg.com/1200x/e0/05/53/e005534fae6501adb82e077161b0a67c.jpg",
      mobile: "https://i.pinimg.com/1200x/e0/05/53/e005534fae6501adb82e077161b0a67c.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/Yra8PgE28lo?si=xfWhnMd08FxLSM34"

  },
  {
    id: 11,
    location: "Cần Thơ, Đồng bằng sông Cửu Long",
    title: "Chợ Nổi Cái Răng",
    subtitle: "Khám phá đời sống miền Tây qua chợ nổi truyền thống với những ghe bầu đầy trái cây tươi ngon.",
    rating: 4.7,
    reviews: 6420,
    duration: "4-5 giờ",
    category: "Văn hóa sông nước",
    price: "từ 380.000₫",
    images: {
      desktop: "https://i.pinimg.com/1200x/74/3f/fb/743ffba55dee0f1b9a5c1800c496c91a.jpg",
      webp: "https://i.pinimg.com/1200x/74/3f/fb/743ffba55dee0f1b9a5c1800c496c91a.jpg",
      tablet: "https://i.pinimg.com/1200x/74/3f/fb/743ffba55dee0f1b9a5c1800c496c91a.jpg",
      mobile: "https://i.pinimg.com/1200x/74/3f/fb/743ffba55dee0f1b9a5c1800c496c91a.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/Yi5ofNj1IJE?si=riqjM56ZWvP5QOVo"

  },
  {
    id: 12,
    location: "Huế, Thừa Thiên Huế",
    title: "Cố Đô Huế",
    subtitle: "Khám phá di sản văn hóa thế giới với Đại Nội hoành tráng và thưởng thức ẩm thực hoàng gia tinh tế.",
    rating: 4.8,
    reviews: 10300,
    duration: "1-2 ngày",
    category: "Di sản văn hóa",
    price: "từ 1.190.000₫",
    images: {
      desktop: "https://i.pinimg.com/1200x/d1/4b/65/d14b65c055c39e4f4d0cb224f41ac79a.jpg",
      webp: "https://i.pinimg.com/1200x/d1/4b/65/d14b65c055c39e4f4d0cb224f41ac79a.jpg",
      tablet: "https://i.pinimg.com/1200x/d1/4b/65/d14b65c055c39e4f4d0cb224f41ac79a.jpg",
      mobile: "https://i.pinimg.com/1200x/d1/4b/65/d14b65c055c39e4f4d0cb224f41ac79a.jpg"
    },
    videoUrl: "https://www.youtube.com/embed/K1ie-Vgss-Q?si=LjD4v_NHTDzgyCmT"

  }
];

// Alternative data structure for different tour categories
export const tourCategories = [
  {
    id: 'cultural',
    name: 'Văn hóa lịch sử',
    color: 'from-amber-500 to-orange-600',
    icon: '🏛️'
  },
  {
    id: 'nature',
    name: 'Thiên nhiên',
    color: 'from-green-500 to-emerald-600',
    icon: '🌲'
  },
  {
    id: 'beach',
    name: 'Biển đảo',
    color: 'from-blue-500 to-cyan-600',
    icon: '🏖️'
  },
  {
    id: 'adventure',
    name: 'Phiêu lưu',
    color: 'from-red-500 to-rose-600',
    icon: '🗻'
  },
  {
    id: 'urban',
    name: 'Đô thị',
    color: 'from-purple-500 to-indigo-600',
    icon: '🏙️'
  },
  {
    id: 'resort',
    name: 'Nghỉ dưỡng',
    color: 'from-pink-500 to-rose-600',
    icon: '🌺'
  }
];

// Featured destinations for quick access
export const featuredDestinations = [
  'Hội An, Quảng Nam',
  'Vịnh Hạ Long, Quảng Ninh', 
  'Sa Pa, Lào Cai',
  'Phú Quốc, Kiên Giang'
];

// Seasonal recommendations
export const seasonalTours = {
  spring: [1, 4, 6, 12], // March-May
  summer: [2, 5, 10], // June-August  
  autumn: [3, 7, 8], // September-November
  winter: [9, 11] // December-February
};

export default heroSlides;