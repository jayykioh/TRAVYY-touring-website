...
const StatCard = ({ stat }) => {
  ...
  const colors =
    colorSchemeMapping[stat.iconColor] || colorSchemeMapping["text-blue-600"];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-base font-semibold text-gray-700">{stat.label}</h3>
        {/* Icon on the right */}
        <Icon className="w-6 h-6" style={{ color: colors.iconBg }} />
      </div>
      ...
      <div className="mb-1">
        <h2
          className={`text-3xl font-bold ${
            stat.variant === "yellow" ? "text-red-600" : "text-gray-900"
          }`}
        >
          {stat.value}
        </h2>
