...
async function getPricesAndMeta(tourId, date) {
  ...
  if (!tour) {
    return {
      unitPriceAdult: 0,
      unitPriceChild: 0,
      meta: { name: "", image: "", departureStatus: "open", seatsLeft: null, seatsTotal: null, found: false },
    };
  }
  const dep = (tour.departures || []).find((d) => (d?.date || "").slice(0, 10) === date);
...
async function getCart(req, res) {
  try {
    ...
    const rows = await CartItem.find({ cartId: cart._id }).sort({ createdAt: -1 });

    const items = [];
    for (const ci of rows) {
      const { meta } = await getPricesAndMeta(ci.tourId, ci.date);
      items.push(mapItem(ci, { seatsLeft: meta.seatsLeft, seatsTotal: meta.seatsTotal }));
    }
...
async function syncCart(req, res) {
  try {
    ...
    for (const inc of incItems) {
      ...
      const { unitPriceAdult, unitPriceChild, meta } = await getPricesAndMeta(tourId, inc.date);
...
async function addToCart(req, res) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (!inc.tourId || !inc.date) throw Object.assign(new Error("BAD"), { status: 400, body: { error: "MISSING_TOUR_OR_DATE" } });

      let tourId;
      try { tourId = new mongoose.Types.ObjectId(inc.tourId); }
      catch { throw Object.assign(new Error("BAD"), { status: 400, body: { error: "INVALID_TOUR_ID" } }); }

      const date = normalizeDate(inc.date);
      const addQty = sumQty(inc.adults, inc.children);
      if (addQty <= 0) throw Object.assign(new Error("BAD"), { status: 400, body: { error: "INVALID_QUANTITY" } });
      ...
      if (seatsLeft != null && current + addQty > seatsLeft) {
        throw Object.assign(new Error("CAPACITY"), { status: 409, body: { error: "EXCEEDS_DEPARTURE_CAPACITY", limit: seatsLeft } });
      }
      ...
      await CartItem.findOneAndUpdate(
        { cartId: cart._id, tourId, date },
        {
          $setOnInsert: {
            selected: true,
            available: meta.departureStatus === "open" && (seatsLeft == null || seatsLeft > 0),
            name: inc.name || meta.name,
            image: inc.image || meta.image,
          },
          $set: {
      ...
      // 6) trả giỏ (kèm seats)
      const rows = await CartItem.find({ cartId: cart._id }, null, { session }).sort({ createdAt: -1 });
      const items = [];
      for (const ci of rows) {
        const { meta: m } = await getPricesAndMeta(ci.tourId, ci.date);
        items.push(mapItem(ci, { seatsLeft: m.seatsLeft, seatsTotal: m.seatsTotal }));
      }
      res.json({ items });
    });
  } catch (err) {
...
async function updateCartItem(req, res) {
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const line = await CartItem.findOne({ _id: itemId, cartId: cart._id }, null, { session });
      if (!line) return res.status(404).json({ error: "ITEM_NOT_FOUND" });

      const nextAdults   = (req.body.adults   != null) ? clamp0(req.body.adults)   : line.adults;
      const nextChildren = (req.body.children != null) ? clamp0(req.body.children) : line.children;

      // seats
      const seatsLeft = await getSeatsLeft(line.tourId, line.date);
      if (seatsLeft != null && sumQty(nextAdults, nextChildren) > seatsLeft) {
        return res.status(409).json({ error: "EXCEEDS_DEPARTURE_CAPACITY", limit: seatsLeft });
      }

      const { unitPriceAdult, unitPriceChild } = await getPricesAndMeta(line.tourId, line.date);
...
async function deleteCartItem(req, res) {
  try {
    ...
    if (!own) {
      console.warn("[DELETE] Not found or not owned:", { userId, cartId: cart._id.toString(), itemId });
