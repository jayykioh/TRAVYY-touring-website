...
const setCache = (k, data, ttl = TTL_MS) => cache.set(k, { data, expireAt: Date.now() + ttl });
...
router.get("/provinces", async (_req, res) => {
  try {
    ...
    const out = Array.isArray(json) ? json.map(p => ({ id: String(p.code), name: p.name })) : [];
  ...
  try {
    ...
    if (!wards.length && Array.isArray(json?.districts)) {
      wards = json.districts.flatMap(d =>
        Array.isArray(d.wards) ? d.wards.map(w => ({ id: String(w.code), name: w.name })) : []
      );
    }
